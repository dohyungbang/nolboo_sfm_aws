...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
year_mon_labels <- format(c(ym(last_month) - months(12:1), ym(last_month)), "%Y년 %m월")
store_n_trend <- 
...
pop_df <- 
  ...
  mutate(value_str = c(paste(formatC(c(sgbiz_pop_fl_13, sgbiz_pop_res_3, sgbiz_pop_work_3), big.mark = ","), "명"),
                       paste(formatC(sgbiz_region_household_n, big.mark = ","), "가구")))
```


### **분석결과:**
#### <span style="color: blue; line-height: 1.5;"> ***목표반경 내 가구 수는 `r formatC(data$sgbiz_region_household_n, big.mark = ",")`가구, 주거인구는  `r formatC(data$sgbiz_pop_res_3, big.mark = ",")`명, 직장인구는  `r formatC(data$sgbiz_pop_work_3, big.mark = ",")`명, 일 평균 유동인구는  `r formatC(data$sgbiz_pop_fl_13, big.mark = ",")`명 입니다.***
