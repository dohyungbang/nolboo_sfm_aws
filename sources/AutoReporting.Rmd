...
t1 <- data.frame(`변수명` = c("홀 면적(평방미터)", "주방 면적(평방미터)", 
                           "테이블 수(개)", "좌석 수(개)", "매장층 수(층)", 
                           "건물전체 층 수(층)", "주차가능 여부(1=가능; 0=불가능)"),
           `입력값`= c(data$nolbu_store_area_hall, data$nolbu_store_area_kitchen, data$nolbu_store_table, 
                      data$nolbu_store_chair, data$nolbu_store_floor, data$nolbu_bldg_floor, data$nolbu_bldg_parking))

t2 <- data.frame(`변수명` = c("일 평균 영업시간(시간)", "월 평균 영업일수(일)", 
                        "예상 직원 수 - 풀타임(명)", "예상직원 수 - 파트타임(명)", 
                        "배달대행 (1=이용; 0=이용안함)", "직접배달 (1=이용; 0=이용안함)",
                        "예상 월 임대료(원)"),
                 `입력값` = c(data$nolbu_store_oper_time, data$nolbu_store_oper_day, data$nolbu_store_emp_full, 
                             data$nolbu_store_emp_part, data$nolbu_delivery_rider_tpl, data$nolbu_delivery_rider_store, 
                             data$nolbu_rent))
...
sales_df <- 
  ...
  mutate(value_str = paste(formatC(round(value, digit = 1), big.mark = ","), "만원"))
```
...
pop_df <- 
  ...
  mutate(value_str = c(paste(formatC(c(sgbiz_pop_fl_13, sgbiz_pop_res_3, sgbiz_pop_work_3), format = "f", big.mark = ","), "명"),
                       paste(formatC(sgbiz_region_household_n, format = "f", big.mark = ","), "가구")))
```


### **분석결과:**
#### <span style="color: blue; line-height: 1.5;"> ***목표반경 내 가구 수는 `r formatC(data$sgbiz_region_household_n, format = "f", big.mark = ",")`가구, 주거인구는  `r formatC(data$sgbiz_pop_res_3, format = "f", big.mark = ",")`명, 직장인구는  `r formatC(data$sgbiz_pop_work_3, format = "f", big.mark = ",")`명, 일 평균 유동인구는  `r formatC(data$sgbiz_pop_fl_13, format = "f", big.mark = ",")`명 입니다.***
...
pop_fl_day <- 
  ...
  mutate(value_str = paste(formatC(value, format = "f", big.mark = ","), "명"))
```
