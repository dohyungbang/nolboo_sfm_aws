...
### **2) 최근 13개월 상권 내 동종업체 월 평균 매출 추이**

```{r, echo=FALSE, message=FALSE, warning=FALSE}
year_mon_labels <- format(c(ym(data$sales_last_month) - months(12:1), ym(data$sales_last_month)), "%Y년 %m월")
sales_trend <- 
  data.frame(label = 1:13,
             label_str = year_mon_labels,
             value = data[,paste0("sgbiz_store_sales_amt_", 1:13)] %>% as.numeric()) %>% 
  mutate(value_str = paste(formatC(value, big.mark = ","), format = "f", digit = 0, "만원"))
```

### **분석결과:**
#### <span style="color: blue; line-height: 1.5;"> ***해당 상권의 최근 13개월 간 동일업종 업체의 월 평균 매출액은 `r ifelse(calCAGR(sales_trend$value) > 0, "증가", "감소")`추세이며, `r year_mon_labels[13]` 기준으로 `r sales_trend$value[13]`개 입니다.***
